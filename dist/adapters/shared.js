...
export function inferCategories(keywords) {
...
export function inferInputsOutputs(text) {
    const inputs = [];
    const outputs = [];
    ...
    if (/\bprompt/i.test(text) || /\bquery/i.test(text) || /\binstruction/i.test(text))
        inputs.push('prompt');
    ...
    if (/\blog/i.test(text) || /\boutput/i.test(text) || /\bexplain/i.test(text))
        outputs.push('console-output');
    return { inputs, outputs };
}
export function extractNameFromContent(content, filePath) {
...
export function extractChainsTo(frontmatter, content) {
