...
export function buildGraph(skills) {
    ...
    for (const a of skills) {
        for (const b of skills) {
            if (a.id === b.id)
                continue;
            if (a.source.agent === b.source.agent && a.name === b.name)
                continue;
            let totalWeight = 0;
            const reasons = [];
            ...
            const outputInputOverlap = a.outputs.filter(o => b.inputs.some(i => i.toLowerCase() === o.toLowerCase()));
            ...
            const hasStrongSignal = outputInputOverlap.length > 0 ||
                triggerSim >= 0.35 ||
                descSim >= 0.25;
...
export function findChains(graph, maxLength) {
    ...
    for (const component of components) {
        ...
        for (const start of startCandidates) {
            dfs([start], new Set([start]), 0, maxLength, outgoing, componentSet, chains, seenChainSets);
        }
    }
    ...
    for (const { chain } of chains) {
        ...
        for (const id of chain)
            usedSkills.add(id);
        if (topChains.length >= 20)
            break;
    }
    return topChains;
}
function dfs(path, visited, totalWeight, maxLength, outgoing, scope, results, seenSets) {
    if (path.length >= 2) {
