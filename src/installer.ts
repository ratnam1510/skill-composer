...
import type { AgentType, ComposerConfig, Skill, ChainDefinition } from './types.js';
...
function generateDynamicSkill(
  baseTemplate: string,
  skills: Skill[],
  catalogSkills: Skill[],
  chains: ChainDefinition[],
): string {
  if (chains.length === 0 && skills.length === 0 && catalogSkills.length === 0) return baseTemplate;
...
async function generateContentForAgent(
  template: string,
  targetAgent: AgentType,
  config?: ComposerConfig,
  dynamic = true,
): Promise<string> {
  if (!dynamic || !config || !(targetAgent in config.agents)) return template;

  const targetConfig: ComposerConfig = {
    ...config,
...
export async function install(
  ...
  config?: ComposerConfig,
  dynamic = true,
): Promise<Array<{ agent: string; path: string; action: string }>> {
  const results: Array<{ agent: string; path: string; action: string }> = [];
