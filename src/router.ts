import type { Skill, SkillGraph, ChainDefinition, ChainManifest, ComposerConfig } from './types.js';
import { makeReferencedSkill } from './catalog.js';
import { buildGraph, findChains } from './graph.js';

function availabilityRank(skill: Skill): number {
  if (skill.availability === 'loaded' || !skill.availability) return 3;
  if (skill.availability === 'discoverable') return 2;
  return 1;
}

function deduplicateSkillsByName(skills: Skill[]): Skill[] {
...
function getDeclaredChains(skills: Skill[]): ChainDefinition[] {
  const nameMap = new Map<string, Skill>();
  for (const s of skills) nameMap.set(s.name.toLowerCase(), s);
  for (const s of skills) nameMap.set(s.id.toLowerCase(), s);

  const chains: ChainDefinition[] = [];

  for (const skill of skills) {
    if (skill.chainsTo.length === 0) continue;
...
function getManualChains(config: ComposerConfig, skills: Skill[]): ChainDefinition[] {
  if (!config.chains || Object.keys(config.chains).length === 0) return [];
...
function getAutoChains(skills: Skill[], maxLength: number): ChainDefinition[] {
...
function deduplicateChains(chains: ChainDefinition[]): ChainDefinition[] {
  const seen = new Set<string>();
  const result: ChainDefinition[] = [];
  ...
  for (const chain of chains) {
    ...
    if (seen.has(reversedKey)) continue;
...
function getReferencedSkills(config: ComposerConfig, skills: Skill[]): Skill[] {
  if (!config.chains || Object.keys(config.chains).length === 0) return [];
...
function formatChainSkill(name: string, missingSkills: string[] = []): string {
  const missing = missingSkills.some(s => s.toLowerCase() === name.toLowerCase());
  return missing ? `\`/${name}\` _(discover first)_` : `\`/${name}\``;
}

export function generateManifest(manifest: ChainManifest): string {
  const lines: string[] = [];
