...
function loadConfig(configPath?: string): ComposerConfig | undefined {
...
function dedupeSkillsForDisplay<T extends { name: string; source: { agent: string }; tokenEstimate?: number }>(skills: T[]): T[] {
...
program
  ...
  .action(async (opts) => {
    ...
    for (const skill of skills) {
      ...
      console.log(
        skill.name.slice(0, nameWidth - 1).padEnd(nameWidth) +
  ...
  .action(async (queryParts: string[] | undefined, opts) => {
    ...
    if (query) {
      ...
      if (remote) {
        ...
        if (cached) console.log(`  Cache: hit, ${cached.tokenEstimate} tokens, expires ${cached.expiresAt}`);
        else console.log('  Cache: miss; run `skill-composer remote fetch <url>` for same-turn cached use');
        console.log('');
        return;
      }
    }
    ...
    for (const skill of results) {
      ...
      if (skill.availability !== 'loaded' && skill.installHint) console.log(`  ${skill.installHint}`);
    }
