...
import type { AgentType, ComposerConfig, ScanResult, Skill, SkillFile } from './types.js';
...
function inferAgentFromPath(path: string): AgentType {
...
function catalogDirs(config: ComposerConfig): string[] {
...
function loadedLookups(loadedSkills: Skill[]): { names: Set<string>; paths: Set<string> } {
  return {
...
export function makeReferencedSkill(name: string, category?: string): Skill {
...
export function rankSkillsForQuery(skills: Skill[], query: string, limit = 20): Skill[] {
  ...
  const scored = skills.map(skill => {
    ...
    if (skill.availability === 'loaded') score += 2;
    if (skill.availability === 'discoverable') score += 1;
